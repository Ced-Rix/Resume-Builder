...
const SignUp = () => {
      ...
      <div class="w-full space-y-8">
        <div class="flex flex-col items-start gap-7">
          ...
          <div class="flex flex-col items-start gap-4">
            <h1 class="text-[1.8rem] font-bold leading-none text-gray-900">Welcome to Rixsume!</h1>
            <span class="text-sm text-gray-600">Create your account and start building ATS-optimized tech resumes with AI-powered tools.</span>
          </div>
        </div>
        <div class="gap-6 grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1">
          <div class="flex flex-col gap-2 col-span-1">
            ...
            <input type="text" id="first-name" placeholder="John"
              class="h-10 w-full rounded-md border border-gray-300 bg-white px-4 text-sm transition duration-300 ease-in-out focus:border-blue-600 focus:ring-2 focus:ring-blue-600" />
          </div>
          <div class="flex flex-col gap-2 col-span-1">
        ...
        <div>
          <input id="subscription-checkbox" type="checkbox"
            class="cursor-pointer rounded border border-gray-300 bg-white text-blue-600 transition duration-300 ease-in-out focus:ring-2 focus:ring-blue-600" />
          <label for="subscription-checkbox" class="ms-2 text-sm text-gray-600">I want to receive emails about events,
            product updates and company announcements.</label>
        </div>
        <div class="flex flex-col gap-6">
          <button
            class="w-full rounded-md border border-blue-600 bg-blue-600 py-2 font-medium text-sm text-white transition duration-300 ease-in-out hover:bg-blue-700 focus:ring-2 focus:ring-blue-600 shadow-lg">Create
            account</button>
          <span class="text-center text-sm text-gray-700">Already have an account? <Link to = '/sign-in'
              class="ml-1 cursor-pointer font-semibold text-blue-600 hover:text-blue-700 underline">Login</Link></span>
        </div>
      </div>
    </form>
  </main>
  <aside class="relative h-80 lg:h-full order-1 lg:order-2">
    ...
    <div class="absolute bottom-10 mx-10 md:bottom-14 md:mx-14">
      ...
      <span class="text-sm text-zinc-300">Rixsume is an open-source AI-powered resume builder designed to help you create ATS-optimized resumes that stand out. Join thousands of developers who've successfully landed their tech jobs with our platform.</span>
    </div>
  </aside>
</div>
  )
